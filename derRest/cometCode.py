...
with sqlite3.connect(DATABASE) as conn:
        cursor = conn.cursor()
        cursor.execute(sql_activate_foreign_keys)
...
### dieser block befüllt main_table
...
with open(SOURCE) as csv_file: # csv datei muss im gleichen verzeichnis liegen
    ...
    with sqlite3.connect(DATABASE) as conn:
        cursor = conn.cursor()
        cursor.execute(sql_create_table_main_table)
        ...
        for line in csv_file:
            try:
                ...
                cursor.execute(sql_insert_main_table, (row[0], row[1], count))
            except Exception as e:
                print("Something wrong with:", line, "Error:", e)
        conn.commit()

### dieser block befüllt weatherconditions_table
# NAs muss ich hier auch noch rauswerfen
...
### dieser block befüllt die temperature tabelle
# NAs muss ich hier auch noch rauswerfen
...
## main_table stationen durch zahlen ersetzen
...
### Aufgabe 4
## für temperatur tabelle höchste mittlere temp ausgeben funktion schreiben
...
temperature_table.mean_temperature, temperature_table.tid, station_table.SID, station_table.Bezeichnung
FROM main_table
