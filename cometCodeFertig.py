...
# temperature_table erstellen mit den Spalten tid (neu dazu), mean_temperature, max_temperature, min_temperature    
    ...
    with sqlite3.connect(DATABASE) as conn:
        ...
        for line in csv_file:
            try:
                ...
                cursor.execute(sql_insert_weatherconditions_table, (row[3], row[4], row[5], row[6]))
            except Exception as e:
                print("Something wrong with:", line, "Error:", e)
        conn.commit()

# temperature_table Daten importieren
with open(SOURCE) as csv_file:
    next(csv_file)
    with sqlite3.connect(DATABASE) as conn:
        cursor = conn.cursor()
        cursor.execute(sql_create_table_temperature_table)

        for line in csv_file:
            try:
                row = line.replace('"', '').strip().split(',')
                cursor.execute(sql_insert_temperature_table, (row[7], row[8], row[9]))
            except Exception as e:
                print("Something wrong with:", line, "Error:", e)
        conn.commit()
...
if maximale_mean_temp:
    ...
    print("Die höchste mittlere Temperatur beträgt:", umgerechnet) # gibt Wert im Terminal aus
else:
...
# Doku konventionen: überschriften drüber, code erklärungen daneben

# not null noch dazu

# in latex bilder einbinden https://www.heise.de/tipps-tricks/LaTeX-Bilder-einfuegen-so-geht-s-4404598.html 2.50 freitag morgen

# in latex unterstriche einfügen https://www.heise.de/tipps-tricks/LaTeX-Unterstrich-einfuegen-so-klappt-s-4425609.html 2.56